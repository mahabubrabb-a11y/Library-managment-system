...
const API_BASE_URL = "http://localhost:5004/api/auth"; // /api/auth যুক্ত করা হয়েছে
const defaultAccounts = [];

const mapUserToFrontend = (user) => {
  if (!user) return null;

  // Database এর _id ফিল্ড থেকে ID নেওয়ার ব্যবস্থা
  ...
  return {
    ...
    academicYear: user.year || user.academicYear || "1st Year",
    semester: user.semester || "Semester 1",
    rollNumber: user.rollNo || user.rollNumber || "",
    // _id থেকে আইডি কাট করে studentId বানানো
    studentId: user.studentId || (userId ? `ST-${String(userId).slice(-6)}` : "ST-000000"),
...
export const AuthProvider = ({children}) =>{
  ...
  // to featch user profile details 
  ...
  //to register
  //to register account & trigger account 
