...
// Backend URL Prefix (book plural নয়, singular)
const API_BOOKS_URL = "http://localhost:5004/api/book";
...
export const LibraryProvider = ({ children }) => {
  ...
  const fetchManualIssues = async () => {
    ...
    try {
      ...
      // Backend bookRouter-এর এন্ডপয়েন্টের সাথে মিল রেখে
      const issuesUrl = isAdmin
        ? `${API_BOOKS_URL}/all-manual-issues`
        : `${API_BOOKS_URL}/student-manual-issues`;
  ...
  const fetchFineSettings = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const fetchedSettings = hydrateFineSettings(settingsData);
  ...
  const returnIssuedRecord = async ({ source, recordId, bookId = null }) => {
    ...
    try {
      // router.put("/return/:id") অনুযায়ী আপডেট করা URL
      const response = await fetch(`${API_BOOKS_URL}/return/${recordId}`, {
        method: "PUT",
        headers: getHeaders(),
      });
  ...
  const applyFineToRecord = async ({ source, recordId, bookId = null, amount }) => {
    ...
    try {
      // router.put("/apply-fine/:id") অনুযায়ী আপডেট করা URL
      const response = await fetch(`${API_BOOKS_URL}/apply-fine/${recordId}`, {
        method: "PUT",
        headers: getHeaders(),
  ...
  const clearFineForRecord = async ({ source, recordId, bookId = null }) => {
    ...
    try {
      // router.put("/clear-fine/:id") অনুযায়ী আপডেট করা URL
      const response = await fetch(`${API_BOOKS_URL}/clear-fine/${recordId}`, {
        method: "PUT",
        headers: getHeaders(),
      });
