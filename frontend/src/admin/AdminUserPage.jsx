...
import { adminUsersPageStyles as s } from "../assets/dummyStyle";
...
function AdminUserPage() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (type === "clear-fine" && clearFineForRecord) {
        await clearFineForRecord(payload);
        triggerToast("Fine cleared successfully.");
      } else if (type === "return-book" && returnIssuedRecord) {
        await returnIssuedRecord(payload);
        triggerToast("Book returned successfully.");
      }
    } catch (error) {
      triggerToast(error?.message || "Action failed.", "error");
    } finally {
  ...
  const exportCsv = () => {
    ...
    link.href = url;
    link.setAttribute("download", `student_summary_${Date.now()}.csv`);
              ...
              <article key={member.email} className={s.studentCard}>
                <div className={s.studentCardHeader}>
                  ...
                  <button
                    ...
                    aria-label={
                      isExpanded ? "Hide student details" : "Show student details"
                    }
                  >
                    <Eye size={18} />
                  </button>
                  ...
                  <div className={s.expandedContainer}>
                    ...
                    <div className={s.booksListContainer}>
                      ...
                      <div className={s.booksList}>
                        {member.history && member.history.length ? (
