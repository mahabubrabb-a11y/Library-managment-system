...
const UserDashboardPage = () => {
  ...
  return (
    <div className={`${s.pageContainer} bg-green-50 min-h-screen`}>
      {/* Hero Section */}
        ...
        {recentBooks.length > 0 ? (
          <div className={s.recentGrid}>
            {recentBooks.map((item, index) => (
              <div
                key={item._id || index}
                className="rounded-[22px] border border-library-ink/10 bg-library-paper/50 p-5"
              >
                <h4 className="font-display text-xl font-semibold text-library-ink">
                  {item.bookTitle || item.title || "Book Title"}
                </h4>
                <p className="mt-1 text-sm text-library-ink/65">
                  Author: {item.author || "N/A"}
                </p>
                <div className="mt-4 flex items-center justify-between text-xs text-library-ink/70">
                  <span>Status: <strong>{item.liveStatus}</strong></span>
